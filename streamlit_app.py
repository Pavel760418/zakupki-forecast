...
**Магазины и остатки.** Продажи по подразделениям (Флагман, Адлер, Сочи…) модуль читает сам.
Если в остатках стоит отбор вроде «Склад = Склад основной1», разбивки по магазинам в остатках нет —
заказ по точкам всё равно считается по продажам, но остатки центрального склада по магазинам не разносятся.
Для учёта остатков по точкам выгрузите остатки **по всем складам/магазинам**, без фильтра на один склад.
    ...
    try:
        with st.spinner("Считаем прогноз и формируем Excel…"):
            ...
            if meta.get("store_grain_fallback"):
                ...
                st.info(
                    "Магазины взяты из **продаж** (подразделения 1С). "
                    "В **остатках** нет разбивки по точкам — в файле обычно только "
                    "«Склад основной1» (см. строку отбора в отчёте 1С). "
                    "Заказ считается по магазинам; остатки центрального склада "
                    "не разносятся по точкам автоматически. "
                    "Чтобы учесть остатки по магазинам, выгрузите остатки без фильтра "
                    "по одному складу / с группировкой по складам точек."
                )
