...
- При детализации по магазинам добавляется **10_Матрица_заказ** (SKU × точка)
  и **11_Перемещение_со_склада** (сначала переместить с «Склад основной», потом заказ поставщику).
...
**Магазины, склад и перемещения.**
- В режиме **по магазинам** модуль сначала смотрит остаток **«Склад основной1»**.
- Предлагает **переместить** товар в точки с дефицитом: приоритет **Флагман**, дальше магазины по объёму продаж SKU.
- Только непокрытую потребность после перемещения включает в **09_Заказ_поставщику**.
- Список перемещений — лист **11_Перемещение_со_склада**.
- Для корректной работы выгружайте остатки **по всем складам/магазинам**, включая центральный.
...
- **Квант** = шт в упаковке/коробе (из наименования `*N` / `/N` или библиотеки `config/quantum_library.py`). Заказ и перемещения округляются **вверх до кванта**; если заказ не нужен (0) — не округляем.
- **Перемещение со склада** (режим «по магазинам»): до заказа поставщику остаток «Склад основной1» распределяется в точки с дефицитом **квантами** (сначала Флагман, затем по продажам). Заказ поставщику на ЦС = **сумма потребности всех магазинов** после перемещений.
...
`11_Перемещение_со_склада` (если по магазинам: что взять с центрального склада до закупки).
...
сначала отрабатывайте 🔴 и 🟠, затем класс A.
    ...
    try:
        with st.spinner("Считаем прогноз и формируем Excel…"):
            ...
            elif meta.get("stock_no_retail_stores") and meta.get("grain") == GRAIN_STORE:
                st.info(
                    "Магазины взяты из **продаж**. В остатках мало розничных точек — "
                    "если есть «Склад основной1», модуль предложит перемещения на лист "
                    "11_Перемещение_со_склада, а непокрытое уйдёт в заказ поставщику."
                )
            if meta.get("grain") == GRAIN_STORE and int(meta.get("transfer_lines", 0) or 0) > 0:
                st.info(
                    f"Предложено перемещений со склада: **{meta.get('transfer_lines')}** строк, "
                    f"**{float(meta.get('transfer_qty_total', 0) or 0):.0f}** шт. "
        ...
        if meta.get("grain") == GRAIN_STORE:
            ...
            if int(meta.get("transfer_lines", 0) or 0) > 0:
                summary += (
                    f" · Переместить со склада: **{meta.get('transfer_lines')}** / "
                    f"**{float(meta.get('transfer_qty_total', 0) or 0):.0f}** шт"
                )
        st.write(summary)
